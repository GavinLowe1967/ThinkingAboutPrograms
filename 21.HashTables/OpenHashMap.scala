...
/** A hash table, representing a mapping.
  * This version does support deletion. */
class OpenHashMap[K: ClassTag, V: ClassTag]
  ...
  /** Array of booleans indicating which slots hold deleted keys. */
  private var deleted = new Array[Boolean](buckets)
   ...
   * true. 
   *   { keys(i) -> values(i) | i <- [0..buckets), filled(i) && !deleted(i) }. */
  ...
  def update(key: K, value: V) = {
    ...
    if(filled(i)){ assert(hashes(i) == h && keys(i) == key); deleted(i) = false }
  ...
  def apply(key: K): V = {
    val h = key.hashCode; val i = find(key, h)
    assert(filled(i) && !deleted(i) && hashes(i) == h && keys(i) == key)
    values(i)
  }

  /** Does the mapping contain key? */
  def contains(key: K): Boolean = {
    val h = key.hashCode; val i = find(key, h); filled(i) && !deleted(i)
  }

  /** Find the value stored for key. */
  def get(key: K): Option[V] = {
    val h = key.hashCode; val i = find(key, h)
    if(!filled(i) || deleted(i)) None
  ...
  def remove(key: K): Boolean = {
    val h = key.hashCode; val i = find(key, h)
    if(!filled(i) || deleted(i)) false
    else{ assert(hashes(i) == h && keys(i) == key); deleted(i) = true; true }
  }
  ...
  private def resize() = {
    ...
    threshold = (buckets*MaxLoadFactor).toInt; mask = buckets-1; theSize = 0
    ...
    val oldDeleted = deleted; deleted = new Array[Boolean](buckets)
    // Copy entries
    for(i <- 0 until oldBuckets; if oldFilled(i) && !oldDeleted(i)){
      val key = oldKeys(i); val h = oldHashes(i); val j = find(key, h)
      assert(!filled(j)); hashes(j) = h; keys(j) = key; 
      values(j) = oldValues(i); filled(j) = oldFilled(i); theSize += 1
    }
  }
